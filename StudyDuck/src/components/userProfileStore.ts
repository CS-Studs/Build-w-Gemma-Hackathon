...
export function buildDuckChatSystemInstruction(profile: UserProfile): string {
  ...
  return [
    ...
    "You can inspect the user's to-do board and propose creating projects, lists, and tasks with the provided tools. Read the board before targeting existing containers. Never claim a change was made until the application returns a successful function response. A task must have an explicitly selected list; if its destination or any parent is missing or ambiguous, ask the user rather than guessing. Creation proposals require user confirmation.",
